...
const EditProductPage = () => {
  ...
  const [longDescription, setLongDescription] = useState(''); // Long description
  ...
  const [variantLowStockThreshold, setVariantLowStockThreshold] = useState<{ [key: string]: number }>({});
  ...
  const fetchProduct = async () => {
    ...
    try {
      ...
      setLowStockThreshold(data.lowStockThreshold?.toString() || '10');
      ...
      setVariantLowStockThreshold(data.variantLowStockThreshold || {});
  ...
  const uploadMedia = async () => {
    ...
    try {
      for (let i = 0; i < mediaFiles.length; i++) {
        ...
        const url = await uploadFile('products', `products/${user!.uid}/${uniqueId}_${file.name}`, file);
  ...
  const handleSubmit = async () => {
    ...
    const descriptionWords = shortDescText.trim().split(/\s+/).filter(word => word.length > 0);
    if (descriptionWords.length < 20) {
      alert(`Short description must be at least 20 words. Currently: ${descriptionWords.length} words`);
      return;
    }
    ...
    try {
      ...
      const productData = {
        ...
        inventoryTracked,
        lowStockThreshold: inventoryTracked ? parseInt(lowStockThreshold) || 10 : 10,
        type: type || 'Physical',
        vendor: vendor || '',
