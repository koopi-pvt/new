...
type MinimalistProductLayoutProps = {
  product: Product;
  theme: Theme;
            ...
            <div className="border-t pt-6" style={{ borderColor: theme.textColor + '20' }}>
              {product.variants.map((variant, index) => (
                <div key={index} className="mb-4">
                  <label className="block text-sm font-medium mb-2 uppercase tracking-wide" style={{ color: theme.textColor }}>
                    {variant.name}
                    {selectedVariants[variant.name] && (
                      <span className="ml-2 text-xs normal-case" style={{ opacity: 0.7 }}>: {selectedVariants[variant.name]}</span>
                    )}
                  </label>
